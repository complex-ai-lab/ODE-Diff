...
class CustomDataset(Dataset):
    ...
    def __getsamples(self, data, proportion, seed):
        x = np.zeros((self.len//self.window, self.window, self.var_num))
        print('real dataset check: ', self.sample_num_total, self.len, self.window, self.var_num)
        # print('x shape: ', x.shape)
        for i in range(0, self.sample_num_total, self.window):
            n = i//self.window
            start = i
            end = i + self.window
            x[n, :, :] = data[start:end, :]
...
class CustomDatasetTrain(Dataset):
    def __init__(
        ...
        super(CustomDatasetTrain, self).__init__()
        assert period in ['train', 'test'], 'period must be train or test.'
        if period == 'train':
            assert ~(predict_length is not None or missing_ratio is not None), ''
        self.name, self.pred_len, self.missing_ratio = name, predict_length, missing_ratio
