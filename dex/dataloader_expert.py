...
class DataGeneratorRoche:
    ...
    def solve(self, init, dose_times, dose_amount):
        ...
        latents = latents + np.random.randn(latents.shape[0], latents.shape[1]) * self.output_sigma
                
        input_factor = np.concatenate((latents, np.ones((1, latents.shape[1]))), axis=0)
        output1 = np.matmul(self.output_coef, input_factor)
        output1 = output1 + np.random.randn(output1.shape[0], output1.shape[1]) * self.output_sigma
        
        latents = np.tanh(latents)
        measurements = np.tanh(output)
        covariate = np.tanh(output1)
        ...
        # actions: D, T
        # latents: D, T
        return measurements, covariate, actions, latents, mask
    ...
    def get_action(self, fixed_time=3, fixed_amount=1.0):
        mid_point = self.n_sample // 2
        
        dose_time = np.full((self.n_sample, 1), fixed_time)
        dose_amount = np.zeros(self.n_sample)
        
        dose_amount[mid_point:] = fixed_amount
    ...
    def generate_data(self):
        ...
        for i in range(self.n_sample):
            measurements, covariate, actions, latents, masks = self.solve(init[i], dose_time[i], dose_amount[i])
            m_list.append(measurements)
            c_list.append(covariate)
        ...
        covariate = self._make_tensor(np.stack(c_list, axis=0).transpose((2, 0, 1)))
        self.actions = self._make_tensor(np.stack(a_list, axis=0).transpose((2, 0, 1)))
        latents = self._make_tensor(np.stack(l_list, axis=0).transpose((2, 0, 1)))
        ...
        self.covariate = (covariate - torch.mean(covariate, dim=(0, 1))) / torch.std(covariate, dim=(0, 1))
        self.latents = (latents - torch.mean(latents, dim=(0, 1))) / torch.std(latents, dim=(0, 1))
    ...
    def export_to_csv(self, output_path="data/simulation_results.csv"):
        time_steps = self.time_dim
        n_patients = self.n_sample
        n_measurements = self.obs_dim
        n_covariates = self.obs_dim
        
        data_rows = []
        
        measurements = self.measurements.cpu().numpy()  # [time, patient, measurement]
        covariate = self.covariate.cpu().numpy()  # [time, patient, covariate]
        actions = self.actions.cpu().numpy()  # [time, patient, 1]
        ...
        for p in range(n_patients):
            for t in range(time_steps):
                ...
                for m in range(n_measurements):
                    row[f'measurement_{m+1}'] = measurements[t, p, m]
                for c in range(n_covariates):
                    row[f'covariate_{c+1}'] = covariate[t, p, c]
                row['dose'] = actions[t, p, 0]
                data_rows.append(row)
        
        df = pd.DataFrame(data_rows)
        
        columns = ['patient_id', 'time'] + \
                 [f'measurement_{i+1}' for i in range(n_measurements)] + \
                 [f'covariate_{i+1}' for i in range(n_covariates)] + \
                 ['dose']
        df = df[columns]
    ...
    def export_to_csv2(self, output_path="data/expert_variables.csv"):
        time_steps = self.time_dim
        n_patients = self.n_sample
        
        data_rows = []
        
        latents = self.latents.cpu().numpy()  # [time, patient, latent_dim]
        ...
        for p in range(n_patients):
            for t in range(time_steps):
                row = {
                    'patient_id': p + 1,
                    'time': t,
                    'Disease': latents[t, p, 0],      # z4
                    'ImmuneReact': latents[t, p, 1],  # z1
                    'Immunity': latents[t, p, 2],     # z5
                    'Dose2': latents[t, p, 3]         # z2
                }
                data_rows.append(row)
        
        df = pd.DataFrame(data_rows)
        
        columns = ['patient_id', 'time', 'Disease', 'ImmuneReact', 'Immunity', 'Dose2']
        df = df[columns]
        
        df.to_csv(output_path, index=False)
        print(f"Expert variables exported to {output_path}")
