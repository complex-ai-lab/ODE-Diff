...
class Trainer(object):
    def __init__(self, config, args, model, dataloader, dataloader_a, dataloader_weight, logger=None):
        super().__init__()
        self.model = model
        ...
        self.dl_a = cycle(dataloader_a['dataloader'])
        self.df_weight = cycle(dataloader_weight['dataloader'])
        self.step = 0
        self.milestone = 0
        self.args = args
        self.logger = logger
        ...
        self.opt = Adam(filter(lambda p: p.requires_grad, self.model.parameters()), lr=start_lr, betas=[0.9, 0.96], weight_decay=1e-4)
    ...
    def save(self, milestone, verbose=False):
        if self.logger is not None and verbose:
            self.logger.log_info('Save current model to {}'.format(str(self.results_folder / f'checkpoint-{self.args.seed}-{milestone}.pt')))
        data = {
            'step': self.step,
        ...
        torch.save(data, str(self.results_folder / f'checkpoint-{self.args.seed}-{milestone}.pt'))

    def load(self, milestone, verbose=False):
        if self.logger is not None and verbose:
            self.logger.log_info('Resume from {}'.format(str(self.results_folder / f'checkpoint-{milestone}.pt')))
        device = self.device
        data = torch.load(str(self.results_folder / f'checkpoint-{self.args.seed}-{milestone}.pt'), map_location=device)
        ...
        with tqdm(initial=step, total=self.train_num_steps) as pbar:
            while step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    ...
                    data_weight = next(self.df_weight).to(device)

                    dropout_rate = 0.2

                    rand_mask = torch.rand(data_a.size(0))
                    mask = (rand_mask <= dropout_rate).unsqueeze(1).unsqueeze(2) 
                    mask = mask.expand(data_a.size(0), data_a.size(1), data_a.size(2)) 
                    data_a[mask] = -1

                    loss = self.model(data, data_a, data_weight, target=data)
