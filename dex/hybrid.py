...
class HybridODE(nn.Module):
    def __init__(self, zx_dim, zy_dim, ze_dim, action_dim, params, learnable_params,device=None):
        ...
        self.seirm = Roche(params, learnable_params, device=self.device)
    ...
    def forward(self, t, state):
        if torch.isnan(state).any():
            raise ValueError("NaN detected in state input")
        
        zx = torch.clamp(state[:, :self.zx_dim], -1, 1)
        zy = torch.clamp(state[:, self.zx_dim:self.zx_dim + self.zy_dim], -1, 1)
        ze = torch.clamp(state[:, self.zx_dim + self.zy_dim:self.zx_dim + self.zy_dim + self.ze_dim], -1, 1)
        a = state[:, -self.action_dim:]  # (1, action_dim)
        ze_input = ze.squeeze()

        # Calculate the derivative of Ze(t)
        dze_dt = self.seirm(t, ze_input, a).unsqueeze(0)
        ...
        dzx_dt = torch.clamp(dzx_dt, -1, 1)

        # Use updated Zx and Ze to calculate the derivative of Zy(t) and apply tanh activation
        combined_state = torch.cat([zy, zx, ze, a], dim=-1)
        dzy_dt = self.linear_zy(combined_state)
        dzy_dt = torch.clamp(dzy_dt, -1, 1)
...
class EncoderLSTM(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim_zx, output_dim_zy, output_dim_ze, device=None):
        ...
        init.normal_(self.g_eta[0].weight, mean=0, std=0.1)
        init.normal_(self.g_xi.weight, mean=0, std=0.1)
        init.normal_(self.g_zeta.weight, mean=0, std=0.1)
...
class HybridDecoder(nn.Module):
    def __init__(self, zx_dim, zy_dim, ze_dim, action_dim, y_dim, x_dim, params, learnable_params, step_size, device=None):
        ...
        init.normal_(self.gy[0].weight, mean=0, std=0.1)
        init.normal_(self.gx[0].weight, mean=0, std=0.1)
        init.normal_(self.gy[2].weight, mean=0, std=0.1)
        init.normal_(self.gx[2].weight, mean=0, std=0.1)
