...
from Models.interpretable_diffusion.model_utils import default, identity, extract, directional_sign_loss, second_order_direction_loss, extrapolated_interval_loss,  align_expert_by_peak_shift_after_t_numpy
...
class Diffusion_TS(nn.Module):
    def __init__(
        ...
        print('dropout: ',attn_pd, resid_pd)
    ...
    def p_mean_variance(self, x, t, a, gt, expf, expc, clip_denoised=True, gamma=1.0, w_v=0.0, w_d=0.0):
        ...
        pred_3 = x_start[:, :4, :]   
        target_3 = gt[:, :4, :]       
        loss_3 = F.mse_loss(pred_3, target_3)  
        grad_3 = torch.autograd.grad(loss_3, x_start, retain_graph=True)[0]
        ...
        value_loss = extrapolated_interval_loss(x_start[:,4:8,:], gt[:,4:8,:], expc[:,4:8,:], expf[:,4:8,:])
        grad_value = torch.autograd.grad(value_loss, x_start, retain_graph=True)[0]


        delta_expert = expc - expf      
        direction_expert = torch.sign(delta_expert)  
        delta_pred = torch.tanh(x_start - gt)                 
        directional_guidance = F.relu(- direction_expert * delta_pred).mean()
        new_grad_cval = torch.autograd.grad(directional_guidance, x_start, retain_graph=True)[0]


        d_delta_pred_dt = delta_pred[:, 1:, :] - delta_pred[:, :-1, :]
        d_delta_expert_dt = delta_expert[:, 1:, :] - delta_expert[:, :-1, :]
        dir_grad_expert = torch.sign(d_delta_expert_dt)  # ∈ {-1, 0, 1}
        grad_direction_guidance = F.relu(-dir_grad_expert * d_delta_pred_dt).mean()
        new_grad_cdir = torch.autograd.grad(grad_direction_guidance, x_start, retain_graph=True)[0]
    ...
    def generate_mts(self, a, gt, expf, expc, w_v, w_d, batch_size=16):
        ...
        return sample_fn((batch_size, seq_length, feature_size), a, gt, expf, expc, w_v, w_d)
